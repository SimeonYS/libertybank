...
class LlibertybankPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `libertybank`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM libertybank WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO libertybank (date, title, link, content)"
